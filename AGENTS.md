...
Single Docker image: PHP 8.3 FPM on `php:8.3-fpm-bookworm`, published as `mmhk/php-docker:8.3-fpm`. No application code, no tests, no lint.
...
Release: push a `v*` tag → GitHub Actions builds multi-arch (amd64/arm64) and pushes to Docker Hub. The image tag is always `8.3-fpm` regardless of git tag.
