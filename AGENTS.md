...
Single Docker image: PHP 8.2 FPM on `php:8.2-fpm-bookworm`, published as `mmhk/php-docker:8.2-fpm`. No application code, no tests, no lint.
...
Release: push a `v*` tag → GitHub Actions builds multi-arch (amd64/arm64) and pushes to Docker Hub. The image tag is always `8.2-fpm` regardless of git tag.
