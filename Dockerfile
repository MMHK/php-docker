...
RUN apt-get update && apt-get install -y tzdata \
 && install-php-extensions @composer gd gmp memcached gettext imagick mcrypt mysqli redis pdo_mysql pdo_pgsql pgsql gmp zstd opcache exif bcmath soap sockets timezonedb zip snmp bz2 shmop ffi \
