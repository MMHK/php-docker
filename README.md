...
> **注意**：PHP 8.0 的 JIT 默认为 `opcache.jit=tracing` 和 `opcache.jit_buffer_size=0`（禁用）。设置 `jit_buffer_size` 后 JIT 才会生效。

### RUN

```
docker run -d --name php-fpm -p 9000:9000 -v /var/www/html:/var/www/html mmhk/php-docker:8.0-fpm
```
